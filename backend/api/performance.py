...
async def get_performance_report_data(
    ...
    regime: str = Query("ALL", pattern="^(Trending|Ranging|Balanced|ALL)$"),
    result: str = Query("ALL", pattern="^(open|win|loss|breakeven|timeout|closed|ALL)$"),
    month: str | None = Query(None, pattern=r"^\d{4}-\d{2}$"),
    search: str | None = Query(None),
    scope: str = Query("active", pattern="^(active|all)$"),
    strategy: str = Query("v2_balanced"),
    simulation_mode: str = Query("fixed_risk", pattern="^(fixed_size|fixed_risk|equity_risk_pct)$"),
    starting_capital: float = Query(1000.0, gt=0),
    capital_per_trade: float = Query(100.0, gt=0),
    risk_per_trade: float | None = Query(10.0, gt=0),
    risk_pct_per_trade: float = Query(1.0, gt=0, le=100),
    fee_pct: float = Query(0.0, ge=0, le=10),
    use_position_multiplier: bool = Query(True),
